...
driver.find_element(By.ID, "user-name").send_keys('standard_user') # поиск локатора поля ввода имени пользователя по ID
print("Input Login")

driver.find_element(By.ID, "password").send_keys("secret_sauce") # поиск локатора поля ввода пароля по ID
print("Input Password")

driver.find_element(By.ID, "login-button").click() # Используем метод .click() для авторизации на сайте
...
try:
    ...
    if 1 <= product_index <= 6:
        value_product_1, value_price_product_1 = get_product_info(name_locators[product_index],
        f"//div[@class='inventory_item'][{product_index}]//div[@class='inventory_item_price']")
        ...
        value_cart_product_1, value_price_cart_product_1 = check_product_info(name_locators[product_index],
        f"//div[@class='inventory_item_price']", value_product_1, value_price_product_1)
        ...
        value_finish_product_1, value_price_finish_product_1 = check_product_info(name_locators[product_index],
        f"//div[@class='inventory_item_price']", value_product_1, value_price_product_1)
        ...
        value_summary_price = driver.find_element(By.XPATH,
        ...
        value_checkout_complete = driver.find_element(By.XPATH,
        "//*[contains(text(), 'Thank you for your order!')]").text
